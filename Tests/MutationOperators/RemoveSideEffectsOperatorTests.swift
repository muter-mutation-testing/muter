...
final class RemoveSideEffectsOperatorSchematatests: XCTestCase {
    ...
    func test_rewriter() throws {
        ...
        let v = RemoveSideEffectsOperator.SchemataVisitor(
            sourceFileInfo: sourceWithSideEffects.asSourceFileInfo
        )

        v.walk(sourceWithSideEffects.code)

        let r = Rewriter(v.schemataMappings).visit(sourceWithSideEffects.code)
