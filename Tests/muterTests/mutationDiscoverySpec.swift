    ...
    override func spec() {
        describe("discoverMutationOperators") {
            it("discovers the mutation operators that can be applied to a Swift file") {
                let operators = discoverMutationOperators(inFilesAt: ["\(self.fixturesDirectory)/sampleForDiscoveringMutations.swift"])
                ...
                expect(operators[2].filePath).to(equal("\(self.fixturesDirectory)/sampleForDiscoveringMutations.swift"))
                expect(operators[3].filePath).to(equal("\(self.fixturesDirectory)/sampleForDiscoveringMutations.swift"))

                expect(operators[0].position.line).to(equal(3))
                expect(operators[1].position.line).to(equal(4))
                expect(operators[2].position.line).to(equal(9))
                expect(operators[3].position.line).to(equal(10))
            }
            ...
            it("doesn't discover any mutation operators in files that don't contain Swift code") {
                ...
                expect(operators.count).to(equal(4))
            }
        }
    }
}
