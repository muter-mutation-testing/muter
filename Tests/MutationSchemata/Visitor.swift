...
final class VisitorTests: XCTestCase {
    func test() throws {
        let code = try SyntaxParser.parse(
            source: """
            #if os(iOS) || os(tvOS)
                import Foo
            #else
        ...
        let visitor = ChangeLogicalConnectorOperator.SchemataVisitor(
            configuration: nil,
            sourceFileInfo: .init(path: "", source: code.description)
        )
        
        visitor.walk(code)
     
        let r = Rewriter(visitor.schemataMappings).visit(code)
    ...
    func test_closureImplicitReturn() throws {
        ...
        let visitor = ImplicitReturnRewriter().visit(code)
